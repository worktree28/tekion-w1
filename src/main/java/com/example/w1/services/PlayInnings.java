  ...
  public void start() {
    ...
    while (this.overs < 20) { // play the over
      for (int i = 0; i < 6; i++) {
        // play the ball
        int runs =
            RunsPerBall.getRuns(this.battingTeam.getPlayers().get(this.battingTeam.getWickets()));
        if (runs != -1) {
          this.battingTeam
              .getPlayers()
              .get(this.battingTeam.getWickets())
              .setRuns(
                  this.battingTeam.getPlayers().get(this.battingTeam.getWickets()).getRuns()
                      + runs);
        ...
        this.battingTeam
            ...
            .get(this.battingTeam.getWickets())
            .setBalls(
                this.battingTeam.getPlayers().get(this.battingTeam.getWickets()).getBalls() + 1);
        ...
        if (this.battingTeam.getStatus() == TeamStatus.LOST) break;
      }
      this.overs++;
    }
    if (!isFirstInnings) {
